<template>
  <n-button @click="showModal = true"> Создать инцидент </n-button>
  <n-modal v-model:show="showModal" preset="dialog" title="Создание инцидента">
    <n-spin :show="isLoad">
      <n-form
        ref="formRef"
        :label-width="80"
      ...
      >
        ...
        <n-form-item label="Адрес (Улица, дом)" path="location_address">
          <n-input
            autosize
            style="min-width: 100%"
            v-model:value="formState.location_address"
        ...
        <n-button @click="handleValidateClick"> Создать </n-button>
      </n-form>
    </n-spin>
  </n-modal>
</template>
...
const formState = ref({
  /*"close_dateq": new Date().setDate(new Date().getDate() + 2),*/ description:
    "",
  location_address: "",
  location_id: "56461",
  org_id: "2def7f07-4676-42cd-b995-39d16854eb73",
  title: "",
});
const rules = {
